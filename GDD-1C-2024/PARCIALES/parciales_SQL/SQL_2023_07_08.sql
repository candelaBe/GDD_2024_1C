...
/*
El uso de CONCAT(YEAR(f1.fact_fecha), '') AS 'Periodo' en 
la consulta tiene un propósito específico relacionado con la uniformidad 
del tipo de dato y el formato de los resultados.

Uniformidad del Tipo de Dato:

La función YEAR(f1.fact_fecha) devuelve un valor de tipo numérico (entero). Sin embargo, en el resto de la consulta (en las partes de semestre y bimestre), el periodo se construye usando CONCAT, lo que genera una cadena de caracteres.
Usar CONCAT(YEAR(f1.fact_fecha), '') asegura que todos los periodos 
(anuales, semestrales y bimestrales) sean tratados como cadenas de texto. 
Esto es importante para que el UNION de la consulta funcione correctamente, 
ya que las columnas de los resultados combinados deben tener tipos de datos 
compatibles.
*/
