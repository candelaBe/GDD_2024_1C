...
select 
	...
	isnull(
		(select sum(ifa.item_cantidad) 
		from Item_Factura ifa 
		join Factura fact 
		on ifa.item_tipo=fact.fact_tipo and ifa.item_sucursal= fact.fact_sucursal and ifa.item_numero = fact.fact_numero 
...
(select 1 from Item_Factura it
	 join Factura f2 on f2.fact_numero = it.item_numero and f2.fact_sucursal = it.item_sucursal and f2.fact_tipo = it.item_tipo
	where YEAR(f2.fact_fecha) = YEAR(f.fact_fecha) and f2.fact_cliente = c.clie_codigo ) and 
exists
	(select 1 from  Item_Factura it2
	 join Factura f3 on f3.fact_numero = it2.item_numero and f3.fact_sucursal = it2.item_sucursal and f3.fact_tipo = it2.item_tipo
